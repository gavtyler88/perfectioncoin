...
The RPM spec file provided here is for PerfectionCoin 0.12.0 and builds on CentOS
...
reference the release tarball available from https://Perfectioncoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[PerfectionCoin git repository](https://github.com/gavtyler88/perfectioncoin) but are not part of
...
in the perfectioncoin git repository. At present only an SVG version of the PerfectionCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the perfectiioncoin
...
needed when 0.12.1 is released, a proper fix is already in the PerfectionCoin git
master branch.
...
idea to build PerfectionCoin against a static version of that library compiled
...
The `perfectioncoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `perfectioncoin-server` package. At this point, most RPM based
...
The `perfectioncoin-server` package should have SELinux support. How to properly do
