...
WORKINGDIR="/tmp/perfectioncoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://perfectoincoin.org"
HOST2="https://perfectioncoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="perfectioncoin-core-"
RCVERSIONSTRING="rc"
...
if [ $? -ne 0 ]; then
   echo "perfectioncoin.org failed to provide signature file, but perfectioncoin.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "perfectioncoin.org and bitcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the PerfectionCoin binary release signing key installed?"
   fi
